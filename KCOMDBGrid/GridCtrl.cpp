...
CGridCtrl::CGridCtrl(int nRows, int nCols, int nFixedRows, int nFixedCols)
{
	...
	m_nCellHeight = m_nHeaderHeight	= 10;		// These will get changed to something meaningful
	...
	memcpy(&m_LogCellFont, &(ncm.lfMessageFont), sizeof(LOGFONT));
	memcpy(&m_LogHeaderFont, &(ncm.lfMessageFont), sizeof(LOGFONT));
...
BOOL CGridCtrl::Create(const RECT& rect, CWnd* pParentWnd, UINT nID, DWORD dwStyle)
{
	...
	TRY {
		...
		m_arColFormat.SetSize(m_nCols);	 // initialize column format
	}
...
void CGridCtrl::EraseBkgnd(CDC* pDC, const CRect & rc) 
{
	CRect  VisRect, rcBounds, rect;
	CBrush FixedBack(GetFixedBkColor()),
		TextBack(GetTextBkColor()),
		Back(GetBkColor());
	
//	if (pDC->GetClipBox(rcBounds) == ERROR) 
//		return;
	rcBounds = rc;
	...
	if (rcBounds.left < nFixedColWidth && rcBounds.top < VisRect.bottom)
		pDC->FillRect(CRect(rcBounds.left, rcBounds.top, 
		nFixedColWidth, VisRect.bottom),
		&FixedBack);
	
	// Draw Fixed rows background
	int nFixedRowHeight = m_nHeaderHeight * m_nFixedRows;
	if (rcBounds.top < nFixedRowHeight && 
		rcBounds.right > nFixedColWidth && rcBounds.left < VisRect.right)
		pDC->FillRect(CRect(nFixedColWidth-1, rcBounds.top,
		VisRect.right, nFixedRowHeight),
		&FixedBack);
	
	// Draw non-fixed cell background
	if (rect.IntersectRect(VisRect, rcBounds)) 
	{
		CRect CellRect(max(nFixedColWidth, rect.left), 
	...
	if (VisRect.right < rcBounds.right) 
		pDC->FillRect(CRect(VisRect.right, rcBounds.top, 
		rcBounds.right, rcBounds.bottom),
		&Back);
	
	// Draw bottom of window below grid
	if (VisRect.bottom < rcBounds.bottom && rcBounds.left < VisRect.right) 
		pDC->FillRect(CRect(rcBounds.left, VisRect.bottom,
		VisRect.right, rcBounds.bottom),
		&Back);
}
...
void CGridCtrl::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags) 
{
	...
	{
	case VK_DELETE: 
		if (IsEditable() && !GetItemText(m_idCurrentCell.row, 
			m_idCurrentCell.col).IsEmpty())
		{
...
void CGridCtrl::OnVScroll(UINT nSBCode, UINT /*nPos*/, CScrollBar* /*pScrollBar*/)
{
	...
	case SB_LINEDOWN:
		if (scrollPos < m_nVScrollMax)
		{
			...
			rect.top = m_nHeaderHeight * m_nFixedRows + yScroll;
			ScrollWindow( 0, -yScroll, rect);
			rect.top = m_nHeaderHeight * m_nFixedRows;
			InvalidateRect(rect);
		}
		break;
...
void CGridCtrl::OnDraw(CDC* pDC, const CRect& rcBounds, const CRect& rcInvalid)
{
	...
	EraseBkgnd(pDC, rcBounds);			// OnEraseBkgnd does nothing, so erase bkgnd here.
	...
	VisRect.OffsetRect(rcBounds.left, rcBounds.top);
	int maxVisibleRow = VisCellRange.GetMaxRow(),
		maxVisibleCol = VisCellRange.GetMaxCol();
	
	// draw top-left cells 0..m_nFixedRows-1, 0..m_nFixedCols-1
	rect.bottom = rcBounds.top - 1;
	for (row = 0; row < m_nFixedRows; row++)
	{
		rect.top = rect.bottom+1;
		rect.bottom = rect.top + GetRowHeight(row)-1;
		rect.right = rcBounds.left - 1;
	...
	rect.bottom = rcBounds.top + nFixedRowHeight-1;
	for (row = minVisibleRow; row <= maxVisibleRow; row++)
	{
		...
		if (rect.top > rcBounds.bottom) 
			break;				// Gone past rcBounds
		if (rect.bottom < rcBounds.top)
			continue;			 // Reached rcBounds yet?
	...
	for (row = 0; row < m_nFixedRows; row++)
	{
		...
		rect.right = rcBounds.left + nFixedColWidth - 1;
	...
	{
		int x = rcBounds.left + nFixedColWidth;
		for (col = minVisibleCol; col <= maxVisibleCol; col++) {
			x += GetColWidth(col);
			pDC->MoveTo(x-1, rcBounds.top + nFixedRowHeight);
	...
	{
		int y = rcBounds.top + nFixedRowHeight;
		for (row = minVisibleRow; row <= maxVisibleRow; row++) {
			y += GetRowHeight(row);
			pDC->MoveTo(rcBounds.left + nFixedColWidth, y-1);	
...
BOOL CGridCtrl::RedrawCol(int col)
{
	BOOL bResult = TRUE;
...
// selects rows
void CGridCtrl::SelectRow(int nRowIndex)
{  
	if (!m_bEnableSelection)
		return;
	
	//if (currentCell.row; == m_idCurrentCell.row) return;
	if (nRowIndex < m_nFixedRows)
		return;
	if (!IsValid(nRowIndex, 0))
		return;
	
	SetSelectedRange(nRowIndex, GetFixedColCount(),
		nRowIndex, GetColCount()-1);
}

// selects cells
void CGridCtrl::SelectCell(CCellID currentCell)
{
	if (!m_bEnableSelection) 
		return;
...
void CGridCtrl::OnSelecting(const CCellID& currentCell)
{
	...
	switch(m_MouseMode)
	{
	case MOUSE_SELECT_ROW:   SelectRow(currentCell.row);	 break;
	case MOUSE_SELECT_CELLS: SelectCell(currentCell);	break;
	}
}
...
void CGridCtrl::CutSelectedText()
{
	...
	if (POSITION pos = m_SelectedCellMap.GetStartPosition())
	{
		DWORD key;
		CCellID cell;
		m_SelectedCellMap.GetNextAssoc(pos, key, (CCellID&)cell);
...
BOOL CGridCtrl::CopyTextFromGrid()
{
	USES_CONVERSION;
	
	CString str;
	CCellRange Selection = GetSelectedCellRange();
	if (!IsValid(Selection) || Selection.Count() > 1) return NULL;
	...
	HGLOBAL hMem;
	hMem = GlobalAlloc(GMEM_FIXED | GMEM_ZEROINIT, str.GetLength() + 1);
	if (!hMem)
		return FALSE;
	lstrcpy((TCHAR *)hMem, str);
...
{
	if (!IsValid(cell) || !IsEditable() || !IsClipboardFormatAvailable(CF_TEXT))
		return FALSE;
	...
	CString str = (TCHAR *)::GetClipboardData(CF_TEXT);
	CloseClipboard();
	if (!str.IsEmpty())
		SetItemText(cell.row, cell.col, str);
...
void CGridCtrl::OnUpdateEditPaste(CCmdUI* pCmdUI) 
{
	...
	BOOL bCanPaste = IsValid(cell) && IsEditable() &&
...
BOOL CGridCtrl::MouseOverColResizeArea(CPoint& point) const
{
	if (point.y >= m_nHeaderHeight * m_nFixedRows)
		return FALSE;
...
{  
	...
	else	// in non-fixed col
	{
		...
		for (int col = idTopLeft.col; col < GetColCount(); col++)
		{
			xpos += GetColWidth(col);
			if (xpos > point.x) break;
		}
	...
	// calculate row index
	int fixedRowHeight = m_nHeaderHeight * m_nFixedRows;
...
CCellID CGridCtrl::GetTopleftNonFixedCell() const
{
	...
	int nCol = m_nFixedCols, nRight = 0;
	while (nRight < nHorzScroll && nCol < (GetColCount()-1))
		nRight += GetColWidth(nCol++);
...
CCellRange CGridCtrl::GetVisibleNonFixedCellRange(LPRECT pRect /*=NULL*/) const
{
	...
	// calc bottom
	int bottom = m_nHeaderHeight * m_nFixedRows;
...
void CGridCtrl::ResetScrollBars()
{
	...
	CRect VisibleRect(GetFixedColWidth(), m_nHeaderHeight * m_nFixedRows, rect.right, rect.bottom);
	CRect VirtualRect(GetFixedColWidth(), m_nHeaderHeight * m_nFixedRows, GetVirtualWidth(), GetVirtualHeight());
...
void CGridCtrl::InitCellFont()
{
	...
	{
		m_CellFont.DeleteObject();
		m_CellFont.CreateFontIndirect(&m_LogCellFont);
		...
		m_nCellHeight = tm.tmHeight+tm.tmExternalLeading + 2*m_nMargin;
		if (m_nDefCellWidth <= 0)
...
void CGridCtrl::InitHeaderFont()
{
	...
	{
		m_HeaderFont.DeleteObject();
		m_HeaderFont.CreateFontIndirect(&m_LogHeaderFont);
		...
		m_nHeaderHeight = tm.tmHeight+tm.tmExternalLeading + 2*m_nMargin;
		m_nDefCellWidth = tm.tmAveCharWidth*12 + 2*m_nMargin;
	}
...
BOOL CGridCtrl::SetFixedColCount(int nFixedCols)
{
	ASSERT(nFixedCols >= 0);
...
BOOL CGridCtrl::SetColCount(int nCols)
{
	ASSERT(nCols >= 0);
	
	if (nCols == GetColCount() || nCols > 255) return TRUE;
	...
	for (int i = 0; i < m_nRows; i++)
		...
		if (addedCols > 0)
		{
			...
			for (int col = startCol; col < GetColCount(); col++)
			{
...
int CGridCtrl::InsertCol(LPCTSTR strHeading, 
							UINT nFormat /* = DT_CENTER|DT_VCENTER|DT_SINGLELINE */,
							int nCol  /* = -1 */)
{
	// If the insertion is for a specific column, check it's within range.
	if ((nCol >= 0 && nCol >= GetColCount()) || GetColCount() >= 255)
		return -1;
	...
	else
	{
		...
		m_arColFormat.InsertAt(nCol, (int)0);
		for (int row = 0; row < m_nRows; row++) 
		{
			GRID_ROW* pRow = m_RowData[row];
			if (!pRow) return -1;
			pRow->InsertAt(nCol, (CGridCell *)NULL);
		}
	}
	...
	// Initialise column data
	SetItemText(0, nCol, strHeading);
	m_arColWidths[nCol] = m_nDefCellWidth;//GetTextExtent(strHeading).cx;
	m_arColFormat[nCol] = 0;
	
	if (m_idCurrentCell.col != -1 && nCol < m_idCurrentCell.col)
		m_idCurrentCell.col++;
...
int CGridCtrl::InsertRow(int nRow /* = -1 */)
{
...
BOOL CGridCtrl::DeleteCol(int nCol)
{
	...
	for (int row = 0; row < GetRowCount(); row++) 
	{
		...
		CGridCell* pCell = pRow->GetAt(nCol);
		if (pCell)
			delete pCell;
	...
	m_arColFormat.RemoveAt(nCol);
	m_nCols--;
	if (nCol < m_nFixedCols) m_nFixedCols--;
	
	if (nCol == m_idCurrentCell.col)
		m_idCurrentCell.row = m_idCurrentCell.col = -1;
	else if (nCol < m_idCurrentCell.col)
		m_idCurrentCell.col--;
...
int CGridCtrl::GetColWidth(int nCol) const
{
...
BOOL CGridCtrl::SetColWidth(int nCol, int width)
{
...
//	ASSERT(nRow >= 0 && nRow < m_nRows && height >= 0);
	...
	if (nRow < 0 || nRow >= m_nFixedRows || height < 0)
...
int CGridCtrl::GetFixedColWidth() const
{
	int nWidth = 0;
...
BOOL CGridCtrl::AutoSizeCol(int nCol)
{
	ASSERT(nCol >= 0 && nCol < m_nCols);
...
void CGridCtrl::AutoSize()
{
	...
	for (nCol = 0; nCol < nNumCols; nCol++)
		for (int nRow = 0; nRow < nNumRows; nRow++)
		{
			...
			if (size.cy > (int) GetRowHeight(nRow)) SetRowHeight(nRow, size.cy);
		}
...
void CGridCtrl::ExpandColsToFit()
{
	...
	int nColAdjustment = nDifference / GetColCount();
	
	for (int i = 0; i < GetColCount(); i++)
		m_arColWidths[i] += nColAdjustment;
...
void CGridCtrl::ExpandRowsToFit()
{
	...
	for (int i = 0; i < GetRowCount(); i++)
		SetRowHeight(i, nRowAdjustment + GetRowHeight(i));
	
	if (nDifference > 0)
	{
		int leftOver = nDifference % GetRowCount();
		for (i = 0; i < leftOver; i++)
			SetRowHeight(i, 1 + GetRowHeight(i));
	} 
	else 
	{
		int leftOver = (-nDifference) % GetRowCount();
...
{  
	...
	else 
	{
		...
		y = m_nHeaderHeight * m_nFixedRows;
...
void CGridCtrl::OnMouseMove(UINT nFlags, CPoint point)
{
	...
	{
		if (m_bAllowColResize && MouseOverColResizeArea(point))
		{
...
void CGridCtrl::OnBeginPrinting(CDC *pDC, CPrintInfo *pInfo)
{
	...
	m_nPageHeight = m_LogicalPageSize.cy - m_nHeaderHeight * m_nFixedRows
...
void CGridCtrl::OnPrint(CDC *pDC, CPrintInfo *pInfo)
{
	...
	pInfo->m_rectDraw.bottom = m_nHeaderHeight * m_nFixedRows; 
	PrintColHeadings(pDC, pInfo);
	pDC->OffsetWindowOrg(0, -m_nHeaderHeight * m_nFixedRows); 
...
void CGridCtrl::PrintColHeadings(CDC *pDC, CPrintInfo* /*pInfo*/)
{
	CFont *pOldFont = pDC->SelectObject(&m_PrinterFont);
...
BOOL CGridCtrl::Save(CArchive & ar)
{
	TRY
	{
		int nCols = GetColCount(), nRows = GetRowCount(), nFixedCols = 
			GetFixedColCount(), nFixedRows = GetFixedRowCount();
		ar << nRows << nCols << nFixedCols << nFixedRows << m_nHeaderHeight
			<< m_nCellHeight;

		for (int i = 0; i < nCols; i ++)
			ar << m_arColWidths.GetAt(i) << m_arColFormat.GetAt(i);
...
BOOL CGridCtrl::Load(CArchive & ar)
{
	TRY
	{
		int nCols, nRows, nFixedCols, nFixedRows;
		ar >> nRows >> nCols >> nFixedCols >> nFixedRows >> m_nHeaderHeight 
			>> m_nCellHeight;
		...
		{
			ar >> nWidth >> nFormat;
			m_arColWidths.SetAt(i, nWidth);
			m_arColFormat.SetAt(i, nFormat);
		}
...
void CGridCtrl::OnFixedColClick(CCellID& cell)
{
	return;
}
...
CSize CGridCtrl::GetTextExtent(LPCTSTR str, BOOL bUseHeaderFont /* = TRUE */)
{
	CDC* pDC = GetDC();
	...
	pOldFont = pDC->SelectObject(bUseHeaderFont ? &m_HeaderFont : &m_CellFont);
...
{
	LOGFONT lf = GetItemFont(nRow, nCol);
...
BOOL CGridCtrl::DrawFixedCell(CDC* pDC, int nRow, int nCol, CRect rect, BOOL bEraseBk)
{
	...
	LOGFONT lfnow = GetItemFont(nRow, nCol);
	...
	{
		static LOGFONT lf;
		memcpy(&lf, &lfnow, sizeof(LOGFONT));
		lf.lfWeight = SELECTED_CELL_FONT_WEIGHT;
		memcpy(&lfnow, &lf, sizeof(LOGFONT));
	}
	
	CFont Font;
	Font.CreateFontIndirect(&lfnow);
	...
	if (m_pImageList && Item.iImage >= 0)
	{
		...
		if (m_pImageList->GetImageInfo(Item.iImage, &Info))
		{
			...
			int nImageWidth = Info.rcImage.right-Info.rcImage.left+1;
			m_pImageList->Draw(pDC, Item.iImage, CPoint(rect.left + (rect.Width() - Info.rcImage.right + Info.rcImage.left) / 2, rect.top + (rect.Height() - Info.rcImage.bottom + Info.rcImage.top) / 2), ILD_NORMAL);
			rect.left += nImageWidth+m_nMargin;
		}
	}
	
	DrawText(pDC->m_hDC, Item.szText, -1, rect, m_arColFormat.GetAt(Item.col));
...
BOOL CGridCtrl::DrawCell(CDC* pDC, int nRow, int nCol, CRect rect, BOOL bEraseBk)
{
	...
	Item.state = 0;
	Item.szText = _T("");
	Item.mask = GVIF_TEXT | GVIF_STATE | GVIF_IMAGE;
	Item.row = nRow;
	Item.col = nCol;
    GetItem(&Item);
	
	COLORREF TextBkClr = GetTextBkColor();
	COLORREF TextClr   = GetTextColor();
...
void CGridCtrl::OnEditCell(int nRow, int nCol, UINT nChar)
{
	...
	CCellID cell(nRow, nCol);
	if (!IsValid(cell) || !IsEditable() || !IsCellVisible(nRow, nCol)) 
		return;
...
//	SendMessageToParent(nRow, nCol, GVN_BEGINLABELEDIT);
	...
	if (GetColFormat(nCol) & DT_RIGHT) dwStyle = ES_RIGHT;
	else if (GetColFormat(nCol) & DT_CENTER) dwStyle = ES_CENTER;
...
inline LOGFONT CGridCtrl::GetItemFont(int nRow, int nCol)
{
	return IsCellFixed(nRow, nCol) ? m_LogHeaderFont : m_LogCellFont;
}
